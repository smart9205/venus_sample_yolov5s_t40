 ...
 *For IVS, a Channel performs the functions of a specific algorithm. The specific algorithm type parameter is defined when the Channel is created.
 ...
 *
 * The following is a diagram of a typical two-channel bitstream product applying Bind.
 *
 ...
 *
 **Main stream: Channel0 of FrameSource Bind OSD group. 0, OSD group. 0 Bind Encoder group. 0. Among them:
 *	*OSD Group.0 registers two regions to display timestamp and string information respectively.
 *	*Encoder Group.0 Two channels are registered for H264 encoding and JPEG encoding respectively. Where JPEG encoding channel image size if not equal to the input setting (Channel0 of FrameSource), then scale (Software at T10) to capture at any resolution.
 **Second Stream: FrameSource's Channel1 Bind IVS Group.0，IVS Group.0 Bind OSD Group.1，OSD Group.1 Bind Encoder Group.1.Among them:
