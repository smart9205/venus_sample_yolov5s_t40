 ...
 */
typedef enum {
	IMP_ENC_SEI_MODE_NONE       = 0x00,  /**< Close SEI */
	IMP_ENC_SEI_MODE_INTRA      = 0x01,  /**< Use SEI for frame-I */
	IMP_ENC_SEI_MODE_ALL        = 0x02,  /**< Use SEI for all frame */
} IMPEncoderSEIMode;

/**
 * Define the attr of EncChannel SEI
 */
typedef struct {
	IMPEncoderSEIMode SeiMode;	     /**< Sei mode */
	uint8_t* uuid;			     /**< uuid,the size is fixed at 16Byte, used for data verification */
	uint8_t* userData;		     /**< user data */
	uint8_t  userDataSize;		     /**< user data size,support max 128Byte */
} IMPEncoderSeiAttr;
/**
 ...
 *
 * Set sei attr of the encChn.
 *
 * @param[in] encChn:        Channnel ID.
 * @param[out] SeiAttr: SEI attribute.
 *
 * @retval  >=0		           success.
 * @retval  <0                     failed.
 *
 ...
 *
 * Get sei attr of the encChn.
 *
 * @param[in] encChn:        Channnel ID.
 * @param[out] SeiAttr: size of Stream buffer.
 *
 * @retval  >=0                    success.
 * @retval  <0                     failed.
 *
