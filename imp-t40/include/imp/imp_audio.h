...
/**
 * Define DRC structure..
 */
typedef struct {
   int threshold1;           /** default 50000 **/
   float ratio;             /** 0.3~0.5 **/
   int threshold2;           /** default 100000 **/
   int mode;                /** Mode 1 or 0, 0 is standard mode, 1 is reverse mode, the energy above the threshold is unchanged, and the energy is reduced below the threshold **/
   float supPoint;          /** The initial suppression frequency range ranges from 0 to 1.0, the value multiplied by the maximum frequency band represents the initial suppressionrequency band**/
   float supExp;            /** Suppression parameter, the larger the value, the stronger the suppression **/
 ...
 *
 * Enable audio Intput dynamic_range_compression feature.
 *
 * @param[in] attr need automatic gain of the audio attribute.
 * @param[in] dynamic range compression threshold ratio.
 *
 * @retval 0 success.
 * @retval non-0 failure.
 *
 * @attention only support 8k and 16k samplerate
 */
 ...
 *
 * Disable AI dynamic_range_compression feature.
 *
 * @param no.
 *
 * @retval 0 success.
 ...
 *
 *Set audio input digital gain.
 *
 * @param[in] audioDevId Audio input device number.
 * @param[in] aiChn Audio input channel number.
 * @param[in] dgain Audio channel input digital gain.
 *
 *The range is from [0,255], corresponding to [-97dB to 30dB], and the step length is 0.5dB.
 *
 * @retval 0 success
 * @retval non-0 failure.
 *
 * @remarks none.
 *
 * @attention AiGain ranges from [0,255]. If the input value is less than 0, the value of aiGain will be set to 0. If the value is greater than 255,aiGain is set to 255.aiGain 's rang is [0 ~ 255].
 */
  int IMP_AI_SetDigitalGain(int audioDevId, int aiChn, int dgain);

/**
 * @fn int IMP_AI_GetDigitalGain(int audioDevId, int aiChn, int *dgain)
 *
 *Get audio input digital gain.
 *
 * @param[in] audioDevId Audio input device number.
 * @param[in] aiChn Audio input channel number.
 * @param[out] dgain Audio channel input digital gain.
 *
 *The range is from [0,255], corresponding to [-97dB to 30dB], and the step length is 0.5dB.
 *
 * @retval 0 success
 ...
 */
  ...
  * @param[in] audioDevId Audio output device number.
  * @param[in] aoChn Audio output channel number.
  * @param[in] dgain Audio output gain, range [0 ~ 0xFF], corresponding to [-120dB ~ 7dB], step size 0.5dB.
  ...
  * @attention aoGain ranges from [0,255]. If the input value is less than 0, the dgain value will be set to 0. If the value is greater than 255,dgain is set to 255.
  */
  ...
  * @param[in] audioDevId Audio output device number.
  * @param[in] aoChn Audio output channel number.
  * @param[out] dgain Audio output gain, range [0 ~ 0xFF], corresponding to [-120dB ~ 7dB], step size 0.5dB.
