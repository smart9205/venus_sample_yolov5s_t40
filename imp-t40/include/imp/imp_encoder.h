...
/*For Zeratul*/
/**
 * @fn int IMP_Encoder_DiscardInitialFrame(int encChn, int num)
 *
 * 丢弃编码初始图像
 *
 * @param[in] encChn 编码Channel号,取值范围: [0, @ref NR_MAX_ENC_CHN - 1]
 * @param[in] num 需要丢弃的帧数 [num >= 0]
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 * @remarks 如果不设置，默认丢弃1帧
 *
 * @attention 丢弃FrameSource初始帧，主要为Zeratul平台使用，由于Zeratul是快启，编码图像的第一帧获取很快，但是部分sensor第一帧辉存在问题，需要丢弃
 */
int IMP_Encoder_DiscardInitialFrame(int encChn, int num);
...
typedef struct {
	uint16_t srcframe_width; 		//源图像宽
	uint16_t srcframe_height;		//源图像高
	uint32_t srcframe_virAddr;		//源图像虚拟地址

	bool crop_enable;				//裁剪使能
	uint16_t crop_top;				//裁剪区域左上角Y坐标
	uint16_t crop_left;				//裁剪区域左上角X坐标
	uint16_t crop_width;			//裁剪区域宽
	uint16_t crop_height;			//裁剪区域高

	bool scaler_enable;				//缩放使能
	uint16_t scaler_width;			//缩放宽
	uint16_t scaler_height;			//缩放高

	uint16_t dstframe_width;		//目标图像宽
	uint16_t dstframe_height;		//目标图像高
	uint32_t dstframe_phyAddr;		//目标图像物理地址
	uint32_t dstframe_virAddr;		//目标图像虚拟地址
	uint16_t *resize_tmp_buf;		//缩放暂存buf
} IMPProcessYuvInfo;
...
/**
 * @brief IMP_Encoder_YuvprocessEncode(void *h, IMPProcessYuvInfo dstframe, IMPEncoderYuvOut *encOut);
 *
 * 裁剪,缩放,编码yuv.
 *
 * @param[in] h			编码yuv初始化句柄.
 * @param[in] dstframe	裁剪缩放属性
 * @param[in] encOut	码流结构体.
 *
 * @retval  >=0     		成功.
 * @retval  <0              失败.
 ...
 * @attention 无.
 */
int IMP_Encoder_YuvprocessEncode(void *h, IMPProcessYuvInfo dstframe, IMPEncoderYuvOut *encOut);
