...
typedef struct {
	...
	unsigned int			colType[64];			/**< 主码流时间戳各字符反色标识 */
} IMPOSDFontAttrData;
...
typedef struct{
	int input_batch;  /*处理图片张数*/
	int input_height;
	int input_width;
	int input_channel; /*rgb类型：3  bgra类型：4*/
}InputParam;
...
typedef struct{
	InputParam inputParam;   /*输入参数*/
	OutputParam outputParam; /*输出参数*/
}OpsParam;


/**
 * @fn int IMP_OSD_ResizeRGB_Pic(uint8_t *in_data, uint8_t *res_data, bool align_corners, OpsParam *param);
 *
 * 缩放bgra格式的OSD图片
 *
 * @param[in] in_data 输入缩放前图片地址
 * @param[out] out_data 输出缩放后图片地址
 * @param[in] align_corners 角度方位，默认false
 * @param[in] param 缩放前后图片属性
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 ...
 * @attention 无。
 */
int IMP_OSD_ResizeRGB_Pic(uint8_t *in_data, uint8_t *res_data, bool align_corners, OpsParam *param);

/*For Zeratul*/
/**
 * @fn int IMP_OSD_MemPoolRequest(int poolID,size_t size,const char * name);
 *
 * 根据poolID申请相应的Group内存池，例如poolID为0对应OSD的Group 0
 *
 *
 * @param[in] poolID 内存池ID编号，对应OSD Group编号。最大为4
 * @param[in] size   REME申请大小
 * @param[in] name   内存池名称
 *
 * @retval 0 成功
 * @retval 非0 失败
 *
 * @remarks 在使用IMP_OSD_CreateRgn(NULL),须设为NULL,且osd区域类型为OSD_REG_PIC_RMEM
 * @remarks 必须在系统初始化时进行设置,并且申请的内存区域在rmem上，大小必须大于等于注册在OSD Group上的区域大小总和
 * @remarks IMP_OSD_SetPoolSize不需要再进行设置
 * @attention 无。
 */
int IMP_OSD_MemPoolRequest(int poolID,size_t size,const char * name);
