...
/*
 * This sample is used to test the image zooming function of the IMP_ISP_Tuning_SetAutoZoom interface
 * Where zoom_test array holds the zooming factor to be tested
 * When the ISP frequency is set to 300M, the maximum magnification is between 4-5 times
 */
...
static void *res_get_video_stream(void *args)
{
	...
	float zoom_test[] = {1.5, 2, 2.5, 3, 3.5, 4};
	for (i = 0; i < totalSaveCnt; i++) {
#if 1
		// Sample captures 200 frames by default, and adjusts the magnification every 30 frames
		if ((chnNum == 0) && (i != 0) && (i % 30 == 0)) {
			...
			autozoom.zoom_width[0] = ((int)(FIRST_SENSOR_WIDTH / zoom_test[i/30-1]))&(~1);
			autozoom.zoom_height[0] = ((int)(FIRST_SENSOR_HEIGHT / zoom_test[i/30-1]))&(~1);
			...
			printf("IMP_ISP_Tuning_SetAutoZoom, ret = %d, multiple = %f: autozoom.zoom_width[0] = %d, autozoom.zoom_height[0] = %d\n", 
					ret, zoom_test[i/30-1], autozoom.zoom_width[0], autozoom.zoom_height[0]);
		}
#endif
