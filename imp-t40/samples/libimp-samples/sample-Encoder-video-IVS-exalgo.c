...
static int sample_osd_exalgo_start(int grp_num)
{
	...
	for (i = 0; i < MAX_EXALGO_RECTNUM; i++) {
		...
		grAttrRect.show = 0;
		grAttrRect.layer = i;
		grAttrRect.scalex = FIRST_SENSOR_WIDTH / FIRST_SENSOR_WIDTH_THIRD;
		grAttrRect.scaley = FIRST_SENSOR_HEIGHT / FIRST_SENSOR_HEIGHT_THIRD;
...
static int sample_ivs_exalgo_start(int grp_num, int chn_num, IMPIVSInterface **interface)
{
	...
	param.width = FIRST_SENSOR_WIDTH_THIRD;
	param.height = FIRST_SENSOR_HEIGHT_THIRD;
